...
public class UnitBuilder : MonoBehaviour {
	...
	public static List<string> roles = new List<string>() {"Valiant Palisade","Dawn Ritualist","Zephyr Minstrel","Ruin Bringer","Raven Chakram"};

	public static List<int> openIDs = new List<int>();
	public static List<int> openClans = new List<int>();

	void Start(){
		openIDs = Enumerable.Range(0,names.Length).OrderBy(x => Random.value).ToList();
		openClans = Enumerable.Range(0,clans.Length).OrderBy(x => Random.value).ToList();
		for(int i=0; i<10; i++){
			RelationsTracker.activeClans.Add(new Clan(clans[popClan()]));
		}

		transform.gameObject.GetComponent<RelationsTracker>().initialize(names.Length);
	}
